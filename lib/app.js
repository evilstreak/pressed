...
proto.buildStaticAction = function( action, url ) {
  action = juice.Application.prototype.buildStaticAction.call( this, action, url );
...
/**
 * Posts#by_date( year, [sort] ) -> Array
 * Posts#by_date( year, month, [sort] ) -> Array
 * - year (Number|String): The year to get posts for
 * - month (Number|String): The month to get posts for
 * - sort (Object): Sorting hash to pass to the database
 *
 * Returns all posts published in `year`, or in `month` of `year` if provided.
 * By default these are sorted in order of date published.
 **/
Posts.prototype.by_date = function( year, month, sort ) {
  var from, to;

  // if the second parameter is an object, it's a sort
  if ( typeof month === "object" ) {
    sort = month;
    month = undefined;
  }

  // default sort is ascending date of publishing
  sort = sort || { published : 1 };

  // if month is defined, just that month
  if ( month ) {
    // Date uses a 0-indexed month so needs dropping by 1
    from = new Date( year, month - 1 );
    to = new Date( year, month );
  }
  // otherwise the whole year
  else {
    from = new Date( year, 0 );
    to = new Date( year + 1, 0 );
  }

  return this.by_range( from, to, sort );
}
...
function make_atom_id( domain, published, post_id ) {
  ...
  return "tag:" + domain + "," + year + "-" + month + "-" + day + ":" + post_id;
}

proto.controllers.feed = function() {
  var feed = this.config.feed,
      posts = this.models.Posts.recent( 10 );

  // stitch some atom specific fields into the posts
  posts = posts.map( function( p ) {
    p.link = this.absUrlFor( "/" + p._id );
    p.id = make_atom_id( feed.domain, p.published, p._id );
    p.published = p.updated = p.published.toISOString();
    return p;
  }, this );

  // use the most recently updated post for overall updated timestamp
  feed.updated = posts.reduce( function( prev, next ) {
    return next.updated > prev.updated ? next : prev;
  } ).updated;

  // serve it right
  this.res.headers[ "content-type" ] = "application/atom+xml";
