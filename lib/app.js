...
app.models.Blog = {
  ...
  recent : function( count ) {
    // default to 10 if none provided
    count = count || 10;
    return this.all( { published : { $lte : new Date() } }, { published : -1 } ).slice( 0, count );
  },
...
app.controllers.index = function() {
  ...
  return {
    published : this.models.Blog.all( { published : { $lte : now } }, { published : -1 } ),
    pending : this.models.Blog.all( { published : { $gt : now } }, { published : -1 } ),
    drafts : this.models.Blog.all()
  }
}
...
// means we can use a web request to force a sync
app.controllers.sync = function() {
